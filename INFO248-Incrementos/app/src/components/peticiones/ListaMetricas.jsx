...
class ListaMetricas extends React.Component {    
  ...
  onAprobarClick = () => {
    for(let i=0; i < this.state.idMetricasA.length ; i++){ 
        axios.put(`http://localhost:4000/metricas/setaprobado/${this.state.idMetricasA[i]}-Añadir`)
    }
    for(let i=0; i < this.state.idMetricasD.length ; i++){ 
        axios.put(`http://localhost:4000/metricas/deletemetricas/${this.state.idMetricasD[i]}-Eliminar`)
    }
  }

  onRechazarClick = () => {
    for(let i=0; i < this.state.idMetricasA.length ; i++){ 
        axios.put(`http://localhost:4000/metricas/deletemetricas/${this.state.idMetricasA[i]}-Añadir`)
    }
    for(let i=0; i < this.state.idMetricasD.length ; i++){ 
        axios.put(`http://localhost:4000/metricas/setaprobado/${this.state.idMetricasD[i]}-Eliminar`)
    }
  }
